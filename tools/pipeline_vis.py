...
def extract_result(dc):
    ...
    res = dc
    # Unwrap mmcv DataContainer
    if hasattr(res, 'data'):
        res = res.data
    # Some test-time pipelines (e.g., MultiScaleFlipAug3D) introduce nested
    # singleton lists/tuples. Keep unwrapping until reaching real payload.
    while isinstance(res, (list, tuple)) and len(res) == 1:
        res = res[0]
        if hasattr(res, 'data'):
            res = res.data
    return res

def normalize_img_metas(metas):
    """Normalize img_metas to a plain dict when possible."""
    metas = extract_result(metas)
    while isinstance(metas, (list, tuple)) and len(metas) > 0:
...
def export_ply_with_boxes(points, boxes_3d, labels_3d, color_map, filename):
    ...
    if pts_np.ndim == 1:
        raise ValueError(
            f"Unexpected points shape {pts_np.shape}. "
            "Please check pipeline output structure before visualization."
        )
...
def main():
    ...
    if args.split in ['val', 'test']:
        ...
        # For visualization, drop modality masking to avoid zeroing points/images.
        if hasattr(target_cfg, 'pipeline'):
            target_cfg.pipeline = [p for p in target_cfg.pipeline if p.get('type') != 'ModalMask3D']
    ...
    for sample_idx in sample_indices:
        ...
        # Handle LiDAR-only configurations where 'img' may not exist
        has_images = 'img' in data
        imgs_tensor = None
        img_metas = None
        lidar2img = None
        ...
        if has_images:
            ...
            img_metas = normalize_img_metas(data.get('img_metas', None))
            # 注意：在 cmt 配置中，lidar2img 可能在 img_metas 里
            lidar2img = img_metas.get('lidar2img', None) if isinstance(img_metas, dict) else None
        else:
            print("  Note: No images in data (LiDAR-only mode). Skipping 2D visualization.")
        ...
        # 6. 2D 投影可视化 (skip if no images)
        if not has_images or imgs_tensor is None:
            print("  Skipping 2D projection visualization (no images).")
        else:
            num_views = imgs_tensor.shape[0]
            img_norm_cfg = img_metas.get('img_norm_cfg', cfg.img_norm_cfg) if isinstance(img_metas, dict) else cfg.img_norm_cfg
            ...
            for view_id in range(num_views):
                ...
                if isinstance(img_metas, dict) and 'ori_shape' in img_metas and 'img_shape' in img_metas:
        ...
        # 7. Dump data
        if args.dump:
            img_norm_cfg_for_dump = None
            if isinstance(img_metas, dict):
                img_norm_cfg_for_dump = img_metas.get('img_norm_cfg', getattr(cfg, 'img_norm_cfg', None))
            else:
                img_norm_cfg_for_dump = getattr(cfg, 'img_norm_cfg', None)
            
            extra_meta = dict(
                img_norm_cfg=img_norm_cfg_for_dump,
                img_scale=img_metas.get('img_shape', [None])[0][:2] if isinstance(img_metas, dict) and 'img_shape' in img_metas else None,
