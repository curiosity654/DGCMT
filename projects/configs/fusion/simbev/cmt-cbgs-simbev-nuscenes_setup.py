...
data_root = "data/simbev/nuscenes_trainval"
input_modality = dict(
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            type=dataset_type,
            dataset_type="SimBEV",  # Use SimBEV dataset type
            data_root=data_root,
            split="train",  # SimBEV uses 'train', 'val', 'test' splits
...
optimizer_config = dict(
    ...
    custom_fp16=dict(
        pts_voxel_encoder=False, pts_middle_encoder=False, pts_bbox_head=False
    ),
)

lr_config = dict(
    policy="cyclic", target_ratio=(8, 0.0001), cyclic_times=1, step_ratio_up=0.4
)
...
log_config = dict(
    ...
    hooks=[
        ...
        dict(
            ...
            init_kwargs=dict(
                project="Dataset Generalize Fusion",
                name="cmt_voxel0075_vov_1600x640_cbgs-simbev",
            ),
        ),
    ],
)
